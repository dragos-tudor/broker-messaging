...
partial class InboundTests
{
  [TestMethod] public void redirecting__happy_path__exit()
  {
    RedirectingSignal[] path = [
      RedirectingEntries.Start, ConvertingStates.Success,
  ...
  {
    RedirectingSignal[] path = [RedirectingEntries.Start, ConvertingStates.Invalid, ConfirmingFinalStates.Success];
    RunRedirectingPipeline(path, TerminalActions.Exit);
  }

  [TestMethod] public void redirecting__converting_error__unrecoverable()
  {
    RedirectingSignal[] path = [RedirectingEntries.Start, ConvertingStates.Error];
    RunRedirectingPipeline(path, TerminalActions.Unrecoverable);
  }

  [TestMethod] public void redirecting__redirecting_error__exit()
  {
    RedirectingSignal[] path = [RedirectingEntries.Start, ConvertingStates.Success, RedirectingStates.Error];
    RunRedirectingPipeline(path, TerminalActions.Exit);
  }

  [TestMethod] public void redirecting__confirming_final_error__exit()
  {
    RedirectingSignal[] path = [RedirectingEntries.Start, ConvertingStates.Success, RedirectingStates.Success, ConfirmingFinalStates.Error];
    RunRedirectingPipeline(path, TerminalActions.Exit);
  }
}
