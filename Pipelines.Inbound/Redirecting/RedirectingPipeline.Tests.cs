...
partial class InboundTests
{
  ...
  {
    RedirectingInput[] path = [RedirectingEntry.Start, ConvertingStates.Invalid, ConfirmingFinalStates.Success];
    RunRedirectingPipeline(path, TerminalActions.Exit);
  }

  [TestMethod] public void redirecting__converting_error__unrecoverable()
  {
    RedirectingInput[] path = [RedirectingEntry.Start, ConvertingStates.Error];
    RunRedirectingPipeline(path, TerminalActions.Unrecoverable);
  }

  [TestMethod] public void redirecting__redirecting_error__exit()
  {
    RedirectingInput[] path = [RedirectingEntry.Start, ConvertingStates.Success, RedirectingStates.Error];
    RunRedirectingPipeline(path, TerminalActions.Exit);
  }

  [TestMethod] public void redirecting__confirming_final_error__exit()
  {
    RedirectingInput[] path = [RedirectingEntry.Start, ConvertingStates.Success, RedirectingStates.Success, ConfirmingFinalStates.Error];
    RunRedirectingPipeline(path, TerminalActions.Exit);
  }
}
