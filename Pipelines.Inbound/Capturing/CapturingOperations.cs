...
partial class InboundFuncs
{
  ...
  internal static ValueTask<(TData, CapturingInput, Exception?)>
  ...
  where TData : ICapturingData<TKey, TValue, TMetadata, TConfirmation, TPayload> =>
    operation switch
    {
      { Capturing: not null } => operation.Capturing(services, data, ct).FromResult<TData, CapturingStates, CapturingInput>(static states => states),
      { Verifying: not null } => operation.Verifying(services, data, ct).FromResult<TData, VerifyingStates, CapturingInput>(static states => states),
      { Mapping: not null } => operation.Mapping(services, data, ct).FromResult<TData, MappingStates, CapturingInput>(static states => states),
      { Validating: not null } => operation.Validating(services, data, ct).FromResult<TData, ValidatingStates, CapturingInput>(static states => states),
      { Inserting: not null } => operation.Inserting(services, data, ct).FromResult<TData, InsertingStates, CapturingInput>(static states => states),
      { Confirming: not null } => operation.Confirming(services, data, ct).FromResult<TData, ConfirmingStates, CapturingInput>(static states => states),
      { ConfirmingFinal: not null } => operation.ConfirmingFinal(services, data, ct).FromResult<TData, ConfirmingFinalStates, CapturingInput>(static states => states),
