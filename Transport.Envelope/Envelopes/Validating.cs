...
{
  internal static IEnumerable<string> GetValidationEnvelopeErrors<TKey, TValue, TMetadata, TConfirmation>(
  ...
  internal static string? ValidateEnvelope<TKey, TValue, TMetadata, TConfirmation>(
    IEnvelope<TKey, TValue, TMetadata, TConfirmation> envelope) =>
      IsValidEnvelope(envelope)?
        default:
        JoinValidationErrors(GetValidationEnvelopeErrors(envelope));
}